...
open('edge');assert.equal($('chartMenuFocus').disabled,false);c.focusTaskAtMenu();assert.deepEqual(Array.from(c.viewport.x),[90,110]);assert.deepEqual(Array.from(c.viewport.y),[55,65]);assert.equal(c.selected,'edge');assert.equal($('chartMenu').hidden,true);
