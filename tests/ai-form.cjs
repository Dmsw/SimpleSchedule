...
function reset(){run('resetEditorAI()');for(const [k,v] of Object.entries({taskTitle:'写论文',taskDetails:'',taskCategory:'工作',taskWorkload:'1',taskImportance:'5',taskUrgency:'0',taskDifficulty:'5',taskDeadline:'2026-09-24T12:00'}))node(k).value=v;node('editor').open=true;}
...
async function main(){
 ...
 assert.equal(Object.keys(run('readEditorAIResult(JSON.stringify({workload:-1,importance:"invalid",difficulty:2.5}),null,"")')).length,0);
 ...
 model({urgency:-12.75,importance:200.25});await run("runEditorAI('urgency')");assert.equal(node('taskUrgency').value,'-12.75');assert.equal(node('taskImportance').value,'5');
 assert.equal(run('readEditorAIResult(JSON.stringify({importance:-200.125,urgency:0}),null,"").importance'),'-200.125');
 assert.equal(run('readEditorAIResult(JSON.stringify({importance:-200.125,urgency:0}),null,"").urgency'),'0');
 ...
 assert(html.includes('id="taskImportance" type="number"'));assert(html.includes('id="taskDifficulty" type="range"'));assert(!html.includes('Tavily'));assert(!html.includes('parseDialog'));console.log('PASS: simple extraction, defaults preserved, single-field completion, undo, date/value validation, API errors, stale/cancelled requests, hover/double tap without API calls, original sliders.');
}
