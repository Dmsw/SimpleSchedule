...
只提供“重要性 × 迫切性”：横轴为重要性，纵轴为迫切性（越靠上越迫切）；圆圈半径按固定公式 √(40 × 工时) 显示（SVG 坐标单位），颜色表示困难度。重要性和迫切性均为可编辑的独立数值，不再限制为 1–10，也不再由剩余时间计算。默认两个坐标轴均为 −10 至 +10；支持自由拖动、缩放及自定义数值范围。
...
AI解析和这两个字段的 AI补全使用已有日程作参照，不再让模型直接猜数值：

1. 对全部已有日程按待估计属性排序（不受地图或筛选影响，排除当前正在编辑的日程）。
2. 选择排序中间位置的日程；偶数个候选时选择右侧中位项。让 AI 比较当前日程与该参照日程。
3. 当前日程更大则仅保留数值严格更大的候选，更小则仅保留严格更小的候选，再取中位项比较，直到候选为空；相同数值的候选一起排除。
4. AI 判断相当时直接采用参照值；否则依据最终上下界取中点。只有单侧边界时，向外增加或减少边界绝对值的 10%，最小步长为重要性 1、任务量 0.1 小时。
5. 某个参照无法判断时跳过它，继续比较剩余候选；无参照或所有比较均无法判断时保留原值。页面会区分无参照、无法判断、精度限制，以及估计值与原值相同。工时仍限 0.1–100000 小时并保留一位小数；精度或范围无法表达比较结果时保留原值。

两项分别比较，过程中显示轮次与剩余候选数，不追问用户。结果只填入表单，检查并保存后才修改日程，沿用撤销与停止功能。比较中会发送当前日程信息与本轮参照日程的完整细节，以及当前日期和时区。每次请求只含一个参照，整个流程可能调用多次 API。总超时 180 秒；请求失败、输入或参照日程变化时不应用本次结果。

运行 `node tests/ai-median.cjs` 验证算法和表单接入；使用模拟模型，真实 API 效果需要自行试用。

### 日程上下文与数据范围

汇报、今日建议和普通问答请求附带**全部日程**的题目、细节、类别、重要性、迫切性、困难度、预计工时、截止时间和完成状态，包括已完成任务；页面筛选不会缩小发送范围。表单 AI解析/AI补全发送本次文本及当前日程已确定的字段；估计重要性或任务量时，还会逐轮发送所选参照日程的信息与完整细节。新建表单中尚未确认的默认参数不作为推断依据。
...
选中一个日程，右键菜单选择 **连接**，然后点击另一个圆圈或标签完成连接（手机端长按打开菜单）。进入连接模式会显示跟随指针的黑色虚线，完成后显示黑色实线。只能单选起点，禁止自身连接和重复连接。

- 按 Esc、点击“取消连接”或地图外部可取消；取消不会修改数据。
- 两端都在当前可见范围内时显示连线，线条位于圆圈后方，端点随圆圈移动及大小变化更新。
- 连接完成后可以通过提示中的撤销恢复；删除日程后对应连线隐藏，撤销删除会恢复连接。
- 连接随日程保存在本地，编辑日程保留连接；导出和导入也包含连接。相同 ID 的日程仍采用备份内容覆盖。
- 当前备份升级为版本 6，仍可读取旧版本；旧版应用不能读取版本 6，请使用此分支或后续支持连接的版本。

运行 `node tests/task-connections.cjs` 检查连接操作、取消、去重、绘制、存储及备份兼容；测试使用模拟 DOM。
...
| 本地存储项 | 内容 |
| --- | --- |
| `focus-schedule-v1` | 日程数据；当前导出格式为版本 6，包含日程连接 |
...
- 支持版本 1、2、4、5、6 的备份；旧备份没有连接字段时按无连接读取。版本 1 的困难度默认为 5、细节模式默认为纯文本。旧版 urgency 字段被忽略，重要性限制为 −10～10。
...
- 拖动时仅预览当前圆圈，松开才保存。已禁用按可见任务动态归一化：圆圈半径使用固定公式 √(40 × 工时)，例如 10 小时半径 20、40 小时半径 40。筛选、平移、其他任务进入或离开视野不再改变当前任务的圆圈大小，边缘拖动后也不会回弹。
