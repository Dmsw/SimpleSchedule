...
地图工具栏点击 **✦ AI 优化重要性**，使用已保存的 API 设置，综合全部日程的题目、完整细节、类别、工作量、困难度、截止日期及完成状态，统一评估重要性。包含筛选或地图范围外以及已完成的日程，每次请求附带当前日期与时区；相关内容会发送至所配置的模型服务。

结果仅调整重要性（支持正负小数），自动保存并适配横轴，日期范围保持不变。信息不足时提示模型保留原值。可点击 **停止** 取消请求，或 **撤销优化** 恢复本次调整；撤销时保留之后手动改过的重要性。若请求期间日程发生变化、正在编辑或模型返回不完整/无效数据，整批结果不应用。

逻辑测试：`node tests/importance-ai.cjs`，使用模拟 API；真实模型效果需通过已配置的服务验证。
...
接口采用一次性返回，尚不支持流式显示和自动重试；AI 优化重要性支持停止请求。
