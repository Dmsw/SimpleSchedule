...
- 地图右键“创建日程”后，立即进入新标签的标题输入；通过表单新建并保存后同样定位到标签。新日程被筛选隐藏时，会清除搜索和类别筛选、调整状态筛选，使其可见。
- 框选一个或多个日程后按 **Delete**，确认后删除；提示中提供撤销，撤销后恢复选择。右键删除与键盘删除共用同一逻辑。仅悬停不会触发键盘删除。
- 单选日程后按 **F2** 可直接修改标题；**Enter** 保存，**Esc** 取消修改，焦点回到标签。点击编辑框外部保存标题。
- 地图空闲时按 **Esc** 清除选择；打开菜单或正在拖动时，Esc 优先处理当前操作。
- 输入框、可编辑文本、中文输入法组字过程及弹窗中的 Delete 不会删除日程。长按 Delete 不会连续弹出删除确认。
- 标题编辑期间若日程被其他窗口改名或删除，本次输入不会覆盖新数据。

测试：`node tests/interaction-flow.cjs`（模拟 DOM，覆盖删除、撤销、输入保护、自动焦点与标题编辑）。
